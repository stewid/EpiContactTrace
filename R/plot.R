...
setMethod("plot",
          ...
          function(x, ...) {
              ...
              if (!is.null(tree$ingoing)) {
                  tree$ingoing <- position_tree(tree$ingoing,
                                                orientation = "South")
                  tree$ingoing$bg <- ifelse(tree$ingoing$level > 0,
                                            "white", "black")
                  tree$ingoing$pch <- 21
              ...
              if (!is.null(tree$outgoing)) {
                  ...
                  tree$outgoing$bg <- ifelse(tree$outgoing$level > 0,
                                             "gray", "black")
              ...
              if (!is.null(vertices)) {
                  plot(y ~ x, data = vertices, frame.plot = FALSE,
                       axes = FALSE, ann = FALSE, type = "n")

                  if (!is.null(edges_in)) {
                      arrows(edges_in$x0, edges_in$y0, edges_in$x1,
                             edges_in$y1, length = 0)
                  }

                  if (!is.null(edges_out)) {
                      arrows(edges_out$x0, edges_out$y0, edges_out$x1,
                             edges_out$y1, length = 0)
                  }
